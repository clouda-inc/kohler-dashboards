...
export class Search extends JanusClient {
  ...
  public getPorudctByProductId = async (productId: string) => {
    const productByIdUrl = `http://${this.context.account}.vtexcommercestable.com.br/api/catalog_system/pub/products/search?fq=productId:${productId}`

    const res = await this.http.getRaw(productByIdUrl, {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `bearer ${this.context.authToken}`,
      },
